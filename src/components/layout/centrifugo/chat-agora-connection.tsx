...
export default function ChatAgoraConnection() {
  ...
  useEffect(() => {
    if (id) {
      ...
      sub.on("subscribed", () => {
        // console.log("Subscription confirmed Buzz centrifugo", sub);
      });

      sub.on("publication", async (ctx: any) => {
        const payload = ctx?.data;

        // console.log("chat agora centrifugo", payload);
  ...
  }, [id, connectUrl, dispatch, channelDetails]);
