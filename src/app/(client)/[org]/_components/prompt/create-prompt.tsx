...
export default function CreatePromptsModal({ open, setOpen }: PromptProps) {
  ...
  const handleSubmit = async () => {
    ...
    const res = await PostRequest(`/agents/${id}/prompts`, payload);
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogContent className="sm:max-w-[425px] md:max-w-[500px] rounded-xl px-0 py-6">
        ...
        <DialogFooter className="pt-6 flex justify-end gap-3 px-6 border-t">
          ...
          >
            Add Prompts {buttonLoading && <Loading />}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
