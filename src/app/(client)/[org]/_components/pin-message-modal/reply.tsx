...
const PinReplyMessageDialog = ({ open, setOpen }: any) => {
  ...
  const handleDelete = async () => {
    ...
    await DeleteSavedMessage(`/channels/pin/${id}/message/${threadReply?.id}`);
    setOpen(false);
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogContent className="sm:max-w-[550px] rounded-lg p-0 overflow-hidden">
        ...
        <div className="px-4 sm:px-6 pt-4 text-sm text-gray-700">
          Are you sure you want to remove this pinned item.
        </div>

        <div className="px-4 sm:px-6 py-4 max-h-[400px] overflow-auto">
        ...
        <DialogFooter className="mb-5 px-4 sm:px-6 pb-4 pt-2">
          <Button
            variant="outline"
          ...
          <Button
            className="mb-5 bg-rose-600 hover:bg-rose-700 text-white text-sm px-4 py-2 rounded-md shadow-sm"
...
export default PinReplyMessageDialog;
