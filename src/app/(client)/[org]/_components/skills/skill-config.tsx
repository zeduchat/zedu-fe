...
import { CredentialDropdown } from "../../samp/credentials-dropdown";
...
export default function N8nEditorPlayground() {
  ...
  const { skill, skillLoading } = state;
  const skillConfig = skill?.config.properties || [];
  ...
  const onClose = () => {
    dispatch({ type: ACTIONS.SKILL_SIDEBAR, payload: false });
  };
    ...
    <div className="flex flex-col h-full bg-white z-50 overflow-hidden">
      ...
      {skillLoading ? (
        ...
        <div className="overflow-y-auto p-4 [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar-thumb]:bg-gray-300">
          ...
          {skill?.config?.credentials && <CredentialDropdown />}
