...
import { Star, MessageCircleIcon, CheckCircle2, Copy } from "lucide-react";
import { useParams, useRouter } from "next/navigation";
import { GetRequest, PatchRequest, PostRequest } from "~/utils/new-request";
...
export default function ColleageProfile() {
  ...
  const handleRoute = async () => {
    ...
    if (agent?.id) {
      localStorage.setItem("channelName", agent?.name);
    }
    ...
    if (res?.status === 200 || res?.status === 201) {
      router.push(
        `/${orgSlug}/home/colleagues/${res?.data?.data?.channel_id}/${res?.data?.data?.participant_id}`
      );
    }
  };
