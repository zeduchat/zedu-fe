describe("e2e tests for change password", () => {
  it("verify change password", () => {
    ...
    cy.get('input[name="currentPassword"]').type("UserLast1234*");
    cy.get('input[name="newPassword"]').type("Password12**");
    cy.get('input[name="confirmPassword"]').type("Password12**");
    cy.get('button:contains("Saving...")').click();
